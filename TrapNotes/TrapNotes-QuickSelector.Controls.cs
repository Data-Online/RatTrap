...
// TrapNotes_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
namespace RatTrap.UI.Controls.TrapNotes_QuickSelector
{
...
// Base class for the SelectorTableControlRow control on the TrapNotes_QuickSelector page.
...
// Base class for the SelectorTableControl control on the TrapNotes_QuickSelector page.
...
public class BaseSelectorTableControl : RatTrap.UI.BaseApplicationTableControl
{
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
        ...
        {    
            ...
            {
                BaseColumn col = TrapNotesTable.GetColumnByName(field);
            ...
            if (includeFormulaAndDFKAParameter) 
            {
                if (formula != "")
                {
                    foreach (BaseColumn col in MiscUtils.GetFormulaFieldList(formula, TrapNotesTable.Instance.TableDefinition))
                    {
                        if (col != null && !columnList.Contains(col))
                            columnList.Add(col);             
                    }
                ...
                {
                    BaseColumn col = TrapNotesTable.GetColumnByName(displayFieldName);
                    if (col != null && !columnList.Contains(col))
                        columnList.Add(col);             
                }
            }
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.Search, this.GetFromSession(this.Search)), true, false);
		        ...
		        if (!string.IsNullOrEmpty(this.Page.Request["dependOnField"]) && this.Page.Request["dependOnVal"] != null) {
			          BaseColumn dependOnCol = TrapNotesTable.GetColumnByName(this.Page.GetDecryptedURLParameter("dependOnField"));
                if (dependOnCol != null){                
      ...
      foreach(BaseColumn col in cols)
      {
        ...
        public virtual string[] GetAutoCompletionList_Search(String prefixText,int count)
        {
            ...
            if(count==0) count = 10;
            RatTrap.Business.TrapNotesRecord[] recordList  = TrapNotesTable.GetRecords(filterJoin, wc, null, 0, count, ref count);
            String resultItem = "";
            if (resultItem == "") resultItem = "";
            foreach (TrapNotesRecord rec in recordList ){
    ...
    this.ViewState["Page_Size"] = this.PageSize;
        ...
        public virtual void AddButton_Click(object sender, EventArgs args)
        {
            ...
            string url = @"../TrapNotes/Add-TrapNotes.aspx?TabVisible=False&SaveAndNewVisible=False";
...
#region "Helper Functions"
        ...
        public virtual SelectorTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (SelectorTableControlRow[])((new ArrayList()).ToArray(Type.GetType("RatTrap.UI.Controls.TrapNotes_QuickSelector.SelectorTableControlRow")));
